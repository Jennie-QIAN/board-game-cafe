...
getObjectFromBgg(baseBggApiUrl + collectionParam)
    ...
    .then(bggGameIds => bggGameIds.forEach(bggGameId => {
        getObjectFromBgg(baseBggApiUrl + gameIdParam + bggGameId)
            .then(data => {
                const { 
                    name: nameList, 
                    id: bggId, 
                ...
                Game.create(game)
                    .then()
                    .catch(err => console.log(`An error occurred while creating a game fetched from BGG API: ${err}`));
            })
            .catch(err => console.log(err));
    }))
    .catch(err => console.log(err));
